...
export const SnippetsPage: React.FC<SnippetsManagerProps> = ({}) => {
	...
	useEffect(() => {
		...
		if (activeSessions.length > 0) {
			setSelectedSession(activeSessions[0]?.id as string);
		}
	}, [activeSessions]);

	const handleRunSnippet = useCallback((snippetId: string) => {
		...
		//todo: run snippet
		//   ipcRenderer
		//     .invoke('snippets:run', { snippetId, sessionId })
		//     .then(() => {
		//       toast({
		//         title: 'Snippet Executed',
		//         description: 'Snippet was executed successfully',
		//       });
		//     })
		//     .catch((error: any) => {
		//       console.error(error)
